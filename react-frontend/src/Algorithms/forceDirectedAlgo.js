...
export function forceDirectedLayout(vertices,edges,graph_distancex, graph_distancey, iterations, threshold, constant){
  ...
  const epsilon = threshold === undefined? 0.01: threshold;
  const delta = constant === undefined? 2: constant;

  //make copies of input
  let new_vertices = vertices.slice();
  let new_edges = [];
  ...
  for(let i = 0; i < new_vertices.length; i++){
    for(let j = 0; j < new_vertices.length; j++){
      for(let k =0; k < edges.length; k++){
        if(vertices[i] === edges[k][0] && vertices[j] === edges[k][1]){
          new_edges.push(new_vertices[i],new_vertices[j]);
        }
      }
    }
  }
  ...
  while(t<K && maxFvt > epsilon){
    ...
    for(let i =0; i < new_vertices.length; i++){
      ...
      for(let j = 0; j < new_edges.length; j++){
        ...
        if(new_vertices[i] === new_edges[j][0] && new_vertices[i] !== new_edges[j][1]){
          const calcs = fattract(new_edges[j][0], new_edges[j][1]);
          // console.log(calcs);
        ...
        //vertices should attract
        if(new_vertices[i] === new_edges[j][1] && new_vertices[i] !== new_edges[j][0]){
          const calcs = fattract(new_edges[j][0], new_edges[j][1])
          f[0] += calcs[0]; // should be two dimensional
          f[1] += calcs[1];
